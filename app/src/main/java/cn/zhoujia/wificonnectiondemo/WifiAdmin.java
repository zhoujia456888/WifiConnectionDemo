...
/**
 * Created by Zhoujia on 2016/7/14.
 */
...
public abstract class WifiAdmin {
    ...
    public WifiAdmin(Context context) {
        ...
        KLog.v(TAG, "getIpAddress = " + mWifiInfo.getIpAddress());
    }
    ...
    public void addNetwork(String ssid, String passwd, int type) {
        if (ssid == null || passwd == null || ssid.equals("")) {
            KLog.e(TAG, "addNetwork() ## nullpointer error!");
            return;
        }

        if (type != TYPE_NO_PASSWD && type != TYPE_WEP && type != TYPE_WPA) {
            KLog.e(TAG, "addNetwork() ## unknown type = " + type);
        }
    ...
    private BroadcastReceiver mBroadcastReceiver = new BroadcastReceiver() {
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            if (intent.getAction().equals(WifiManager.RSSI_CHANGED_ACTION)) {
                KLog.d(TAG, "RSSI changed");

                //有可能是正在获取，或者已经获取了
                KLog.d(TAG, " intent is " + WifiManager.RSSI_CHANGED_ACTION);
    ...
    private synchronized void register() {
        KLog.v(TAG, "register() ##mHaveRegister = " + mHaveRegister);
    ...
    private synchronized void unRegister() {
        KLog.v(TAG, "unRegister() ##mHaveRegister = " + mHaveRegister);
...
//      mTimer.schedule(mTimerTask, 0, 20 * 1000);// 20s
    ...
    public WifiConfiguration createWifiInfo(String SSID, String password, int type) {
        ...
        KLog.v(TAG, "SSID = " + SSID + "## Password = " + password + "## Type = " + type);
    ...
    public int isWifiContected(Context context) {
        ...
        KLog.v(TAG, "isConnectedOrConnecting = " + wifiNetworkInfo.isConnectedOrConnecting());
        KLog.d(TAG, "wifiNetworkInfo.getDetailedState() = " + wifiNetworkInfo.getDetailedState());
        if (wifiNetworkInfo.getDetailedState() == DetailedState.OBTAINING_IPADDR
            ...
            KLog.d(TAG, "getDetailedState() == " + wifiNetworkInfo.getDetailedState());
    ...
    private WifiConfiguration IsExsits(String SSID) {
        ...
        if (existingConfigs != null) {
            ...
            System.out.println("[WifiAdmin] IsExsits existingConfigs!=null");
        }
